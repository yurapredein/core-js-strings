...
function isPalindrome(str) {
  const ist = /[\W_]/g;
  const lowRegStr = str.toLowerCase().replace(ist, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentenceSplit.length; i += 1) {
    if (sentenceSplit[i].length > maxWord.length) {
      maxWord = sentenceSplit[i];
    }
  }
  return maxWord;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverseWordArr = arr.map((word) => word.split('').reverse().join(''));
...
function invertCase(str) {
  ...
  const res = arr.map((el) => {
    if (el >= 'a' && el <= 'z') return el.toUpperCase();
    if (el >= 'A' && el <= 'Z') return el.toLowerCase();
    return el;
  });
  return res.join('');
}
